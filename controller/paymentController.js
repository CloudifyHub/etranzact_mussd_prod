...
const processPayment = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // Return success once body is valid
    await saveLog('Payment request validated successfully', body.transactionId, 'success', JSON.stringify(body));
    res.status(200).json({
      status: 'success',
    ...
    // Continue processing logic after response is sent
    (async () => {
      try {
        ...
        if (newTxn) {
          ...
          await newTxn.save({ transaction: t }).catch(() => {});
        }
        await saveLog(`Background processing error: ${err.message}`, body.transactionId, 'failed');
